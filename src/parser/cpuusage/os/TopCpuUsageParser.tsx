...
export default class TopCpuUsageParser {
  public static parseCpuUsage(lines: string[], callback: ParseCpuUsageCallback): void {
    ...
    const loadAverages = TopCpuUsageParser.parseLoadAverages(lines.shift());

    // Tasks: 466 total,   4 running, 462 sleeping,   0 stopped,   0 zombie
    const runningProcesses = TopCpuUsageParser.parseRunningProcesses(lines.shift());
    ...
    const memoryUsage = TopCpuUsageParser.parseMemoryUsage(lines.shift(), lines.shift());
    ...
    //  ... until <EOT>
    const threadCpuUsages = TopCpuUsageParser.parseThreadCpuUsages(lines);

    callback(CpuUsage.fromTop(timestamp, runningProcesses, threadCpuUsages, loadAverages, memoryUsage));
  }
  ...
  private static parseThreadCpuUsages(lines: string[]): ThreadCpuUsage[] {
    ...
    lines
      ...
      .forEach((columns) => {
        ...
        threadCpuUsages.push(new ThreadCpuUsage(id, runningFor, cpuUsage));
      });
