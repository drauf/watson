import { ThreadDump, Thread, ThreadStatus, Lock } from "../types";
import { getDateFromFilename, matchOne, matchMultipleGroups } from "./RegExpUtils";
...
function parseThreadHeader(header: string, threadDump: ThreadDump): void {
    ...
    currentThread.id = parseInt(matchOne(header, NID_PATTERN));
}
...
function stringToThreadStatus(status: string): ThreadStatus {
    let key = status as keyof typeof ThreadStatus;
