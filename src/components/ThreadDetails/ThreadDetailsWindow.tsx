...
const renderStatus = (thread: Thread) => {
  if (thread.status === undefined) {
    return <div>State: <span className="thread-state unknown">UNKNOWN STATE</span></div>;
  }
...
const renderRunningFor = (thread: Thread) => {
  return <div>Running for: {thread.runningFor}</div>;
};
...
const renderWaitingFor = (thread: Thread) => {
  return (
    <div>
      ...
      {thread.lockWaitingFor &&
        <span>
          This thread is waiting for notification on lock [{thread.lockWaitingFor.id}]
...
const renderLocksHeld = (thread: Thread) => {
  return (
    <div>
      ...
      {thread.locksHeld.length > 0 &&
        <span>This thread holds [{thread.locksHeld.map(lock => lock.id).join(', ')}]</span>}
    </div>
  );
};
